...
\ Compatability shim with eForth
: OUT ( val pin -- )
    dup #2 #7 within if              \ pins 2-7 PORTD
        #2 - lshift                  \ bit mask
        swap if
            $002B c@ or $002B c!    \ set bit
        else
            invert $002B c@ and $002B c!  \ clear bit
        then
    else
        #8 - lshift                  \ pins 8+ PORTB
        swap if
            $0025 c@ or $0025 c!
        else
            invert $0025 c@ and $0025 c!
        then
    then ;
...
: INIT-PIN
    $002A c@ %11111100 or $002A c!   \ DDRD pins 2-7 output
    $0024 c@ %00001110 or $0024 c!   \ DDRB pins 9-11 output
;
